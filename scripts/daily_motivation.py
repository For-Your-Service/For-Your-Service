...
def run_git_command(cmd):
    ...
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        cwd="/Workspace/Users/whall4.wh@gmail.com/For-Your-Service",
    )
...
def main():
    ...
    print(
        f"Progress: [{'█' * int(commits_progress / 10)}{' ' * (10 - int(commits_progress / 10))}] {commits_progress:.0f}%"
    )

    print(f"\nLines: {added:,}/{goal_lines:,} {'✅' if added >= goal_lines else '❌'}")
    print(
        f"Progress: [{'█' * int(lines_progress / 10)}{' ' * (10 - int(lines_progress / 10))}] {lines_progress:.0f}%"
    )
