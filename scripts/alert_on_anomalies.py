...
def check_anomalies(spark):
    ...
    for row in zero_df.collect():
        if row["count"] == 0:
            alerts.append(
                {"severity": "CRITICAL", "message": f"Zero jobs ingested from {row['source']}"}
            )
    ...
    if null_pct > 50:
        alerts.append({"severity": "WARNING", "message": f"High salary null rate: {null_pct}%"})
    ...
    if dup_rate > 10:
        alerts.append({"severity": "WARNING", "message": f"High duplicate rate: {dup_rate:.2f}%"})
