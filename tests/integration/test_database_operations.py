...
def spark():
    ...
    return SparkSession.builder.appName("integration_tests").getOrCreate()
...
def test_write_to_bronze_layer(spark):
    ...
    # Verify write
    result = spark.table("veteran_intake.bronze_jobs").filter("job_id = 'TEST001'").count()
...
def test_bronze_to_silver_transformation(spark):
    ...
    silver_df = (
        bronze_df.withColumn("title", F.initcap("title"))
