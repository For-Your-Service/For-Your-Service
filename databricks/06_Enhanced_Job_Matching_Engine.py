...
# DBTITLE 1,📄 Export Results to PDF
# Generate professional PDF report of job matching results
...
output_dir = "/Workspace/Users/whall4.wh@gmail.com/job_reports"
os.makedirs(output_dir, exist_ok=True)

# Generate filename with timestamp
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
applicant_name_clean = veteran_profile['name'].replace(" ", "_")
filename = f"{output_dir}/{applicant_name_clean}_Job_Match_Report_{timestamp}.pdf"
...
story.append(Paragraph("JOB MATCHING REPORT", title_style))
story.append(Spacer(1, 0.3*inch))

story.append(Paragraph(f"<b>Applicant:</b> {veteran_profile['name']}", normal_style))
story.append(Paragraph(f"<b>Report Date:</b> {datetime.now().strftime('%B %d, %Y')}", normal_style))
story.append(Paragraph(f"<b>Generated by:</b> 7 Eagle Group Job Matching System", normal_style))

story.append(Spacer(1, 0.5*inch))

# Summary box
summary_data = [
    ['Total Jobs Analyzed', str(len(jobs_tensor_sorted))],
    ['Best Match Score', f"{jobs_tensor_sorted.iloc[0]['match_score']:.0f}/100"],
    ['Strong Matches (70+)', str((jobs_tensor_sorted['match_score'] >= 70).sum())],
    ['Good Matches (60-69)', str(((jobs_tensor_sorted['match_score'] >= 60) & (jobs_tensor_sorted['match_score'] < 70)).sum())],
]
...
profile_data = [
    ...
    ['Experience', f"{veteran_profile['experience_summary']['total_years']} years ({veteran_profile['experience_summary']['seniority_level']}-level)"],
    ['Location', f"{veteran_profile['location']['target_city']}, {veteran_profile['location']['target_state']}"],
    ['Target Salary', f"${veteran_profile['salary_requirements']['min']:,} - ${veteran_profile['salary_requirements']['max']:,}"],
]
...
# =====================================================================
# 3. RESUME QUALITY FEEDBACK
# =====================================================================

if 'resume_analysis' in globals():
    story.append(Paragraph("RESUME QUALITY ASSESSMENT", heading_style))
    story.append(Spacer(1, 0.1*inch))
    ...
    if score >= 85:
        ...
        score_status = "⭐⭐⭐⭐ GOOD"
        score_color = colors.HexColor('#558b2f')
    elif score >= 55:
        score_status = "⭐⭐⭐ FAIR"
        score_color = colors.HexColor('#f9a825')
    else:
        score_status = "⭐⭐ NEEDS WORK"
        score_color = colors.HexColor('#d84315')
    
    story.append(Paragraph(f"<b>Score:</b> {score}/100 - {score_status}", normal_style))
    story.append(Spacer(1, 0.1*inch))
    
    if analysis['recommendations']:
        story.append(Paragraph("<b>Recommendations to Improve Your Resume:</b>", subheading_style))
        for i, rec in enumerate(analysis['recommendations'], 1):
            story.append(Paragraph(f"{i}. {rec}", normal_style))
    else:
        story.append(Paragraph("✅ Your resume is well-structured with no major issues!", normal_style))
...
story.append(Paragraph("YOUR TOP JOB MATCHES", heading_style))
story.append(Spacer(1, 0.1*inch))

# Display top 10 matches
for rank in range(min(10, len(jobs_tensor_sorted))):
    job = jobs_tensor_sorted.iloc[rank]
    score = job['match_score']
    ...
    if score >= 70:
        priority = "HIGH PRIORITY"
        priority_color = colors.HexColor('#2e7d32')
    elif score >= 60:
        priority = "RECOMMENDED"
        priority_color = colors.HexColor('#558b2f')
    elif score >= 50:
        priority = "OPTIONAL"
        priority_color = colors.HexColor('#f9a825')
    else:
        priority = "LOW PRIORITY"
        priority_color = colors.grey
    
    # Match header
    story.append(Paragraph(f"<b>MATCH #{rank+1} - {priority} (Score: {score:.0f}/100)</b>", subheading_style))
    ...
    job_data = [
        ...
        ['Experience Level', job['seniority_level'].title()],
    ]
    
    if pd.notna(job.get('url')):
        job_data.append(['Apply Here', job['url'][:60] + '...' if len(job['url']) > 60 else job['url']])
    ...
    if (rank + 1) % 3 == 0 and rank < min(10, len(jobs_tensor_sorted)) - 1:
        story.append(PageBreak())
...
next_steps = [
    "1. Review your top matches in this report",
    "2. Apply to HIGH PRIORITY jobs first",
    "3. Customize your cover letter for each application",
]

if 'resume_analysis' in globals() and resume_analysis.get('recommendations'):
    next_steps.extend([
        "4. Update your resume with the recommended improvements",
        "5. Re-run the matching analysis to see your improved scores"
    ])
...
story.append(Spacer(1, 0.3*inch))
story.append(Paragraph("<b>Need Help?</b> Contact 7 Eagle Group for personalized support", normal_style))
story.append(Spacer(1, 0.2*inch))
story.append(Paragraph("<i>Generated by 7 Eagle Group Job Matching System</i>", normal_style))
...
print(f"\n📁 Saved to: {filename}")
print(f"\n📊 Report includes:")
print(f"   • Your profile summary")
print(f"   • Resume quality assessment")
print(f"   • Top {min(10, len(jobs_tensor_sorted))} job matches with details")
print(f"   • Application links and next steps")
print(f"\n💡 Share this report with recruiters or keep it for your records!")
