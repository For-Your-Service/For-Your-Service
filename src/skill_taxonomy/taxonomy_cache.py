...
class TaxonomyCache:
    ...
    def set_onet_occupation(self, onet_code: str, data: Dict):
        ...
        """
        self.onet_cache[onet_code] = {"data": data, "timestamp": datetime.now().isoformat()}
    ...
    def set_normalized_skill(self, skill_name: str, normalized_data: Dict):
        ...
        """
        key = skill_name.lower()
        self.skills_cache[key] = {"data": normalized_data, "timestamp": datetime.now().isoformat()}
    ...
    def clear_expired(self):
        ...
        expired_onet = [
            code
            for code, entry in self.onet_cache.items()
        ...
        expired_skills = [
            skill
            for skill, entry in self.skills_cache.items()
            if self._is_expired(entry.get("timestamp", ""))
        ]
