...
class GapAnalyzer:
    ...
    def analyze(
        ...
        if total_skills > 0:
            ...
            max_score = sum(importance_weights.get(req.get("importance", "Important"), 2.0) 
                          for req in job_requirements)
    ...
    def _generate_recommendations(
        ...
        if missing_skills:
            ...
            recommendations.append(
                f"📚 Start with {top_gap.skill_name} - "
                f"estimated time: {top_gap.estimated_time}"
            )
    ...
    def get_skill_priority_order(self, gaps: List[SkillGap]) -> List[SkillGap]:
        ...
        return sorted(
            gaps,
            key=lambda g: (
                importance_order.get(g.importance, 3),
                g.skill_name
            )
        )
