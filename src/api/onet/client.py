...
class ONetClient:
    ...
    def get_occupation(self, onet_code: str) -> Dict:
        ...
        response = self.session.get(f"{self.BASE_URL}/occupations/{onet_code}", timeout=30)
        response.raise_for_status()
        return response.json()

    def get_skills(self, onet_code: str) -> Dict:
        """Get skills required for occupation"""
        response = self.session.get(f"{self.BASE_URL}/occupations/{onet_code}/skills", timeout=30)
        response.raise_for_status()
        return response.json()
    ...
    def get_abilities(self, onet_code: str) -> Dict:
        ...
        response = self.session.get(
            f"{self.BASE_URL}/occupations/{onet_code}/abilities", timeout=30
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_knowledge(self, onet_code: str) -> Dict:
        ...
        response = self.session.get(
            f"{self.BASE_URL}/occupations/{onet_code}/knowledge", timeout=30
        )
        response.raise_for_status()
        return response.json()
    ...
    def search_occupations(self, keyword: str) -> Dict:
        ...
        response = self.session.get(
            f"{self.BASE_URL}/search", params={"keyword": keyword}, timeout=30
        )
        response.raise_for_status()
        return response.json()
