...
class CareerOneStopClient:
    ...
    def get_veteran_employers(
        ...
        response = self.session.get(
            f"{self.BASE_URL}/veteranemployer/{self.user_id}/{city}/{state}/{radius}",
            timeout=30
        )
    ...
    def get_certifications(self, keyword: str) -> Dict:
        ...
        """
        response = self.session.get(
            f"{self.BASE_URL}/certificationfinder/{self.user_id}/{keyword}",
            timeout=30
        )
        response.raise_for_status()
        return response.json()
    
    def get_american_job_centers(
        self,
        city: str,
        state: str,
        radius: int = 25
        ...
        """
        response = self.session.get(
            f"{self.BASE_URL}/jobcenter/{self.user_id}/{city}/{state}/{radius}",
            timeout=30
        )
        response.raise_for_status()
        return response.json()
