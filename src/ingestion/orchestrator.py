...
class DataOrchestrator:
    ...
    def _initialize_clients(self) -> Dict:
        ...
        if self.config.USAJOBS_API_KEY:
            clients["usajobs"] = USAJobsClient(
                api_key=self.config.USAJOBS_API_KEY, user_agent=self.config.USAJOBS_USER_AGENT
            )

        if self.config.ADZUNA_APP_ID and self.config.ADZUNA_API_KEY:
            clients["adzuna"] = AdzunaClient(
                app_id=self.config.ADZUNA_APP_ID, api_key=self.config.ADZUNA_API_KEY
            )
    ...
    def collect_job_postings(self, keywords: List[str], locations: List[str]) -> List[Dict]:
        """
        ...
        for keyword in keywords:
            for location in locations:
                ...
                if "adzuna" in self.clients:
                    ...
                    try:
                        jobs = self.clients["adzuna"].search_jobs(what=keyword, where=location)
                        all_jobs.extend(jobs)
